...
function cargar_lista_egreso(){
    ...
    if (primera > segunda) {                    
        ...
        var route = "egreso_lista/"+fecha_inicio+"/"+fecha_fin;
        $("#datos").empty();
        $.get(route, function (res) {
        $("#datos").empty();
            $(res).each(function (key, value) {
                tabladatos.append("<tr align=center style='background-color:white' onmouseover='this.style.backgroundColor=\"#F6CED8\"' onmouseout='this.style.backgroundColor=\"white\"'><td>"+value.detalle+"</td><td>"+value.precio+" Bs.</td><td>"+value.fecha+"</td><td><center> <button class='btn btn-primary' data-toggle='modal' data-target='#myModalActualizar' onclick='cargar_egreso("+value.id+","+value.id_categoria+")' >ACTUALIZAR</button>  <button value="+value.id_compra+" class='btn btn-danger' data-toggle='modal' data-target='#myModalAnular' onclick='anular_egreso("+value.id+")' >ANULAR EGRESO</button></center></td></tr>");           
            });
        });  
    } 
}

function anular_egreso(id){
    $("#id_egreso").val(id);
}

function cargar_egreso(id,id_categoria){
    $("#id_egreso_ac").val(id);
    var route = "actualizar_egreso/"+id;
   $.get(route, function (res) {
...
function cargar_select(id) {   //ES EL SELECT DEL ACTUALIZAR
    ...
    $.get("select_egreso/"+id, function (response) {
